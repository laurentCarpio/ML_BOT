# ml_bot/scripts/sageMaker/grid_search_xgb_long.py
"""
Grid search XGBoost — LONG only — stage3 v50

- Dataset : s3://tradebot-config-tokyo/data/stage3/v50-long
- Objectif : binaire (Y=1 => trade long "win" selon THRESH_BPS)
- Sorties :
    * Jobs SageMaker XGBoost (spot)
    * Résumé grid search (AUPRC / F1) sur S3
    * Manifest de déploiement pour infer_xgb.py
    * Utilitaires pour évaluer TEST et générer des thresholds par poche (tf×side_num)
"""
...
# ✅ version de dataset (facile à changer ici)
VERSION = "v50"

# ✅ dataset v50, long-only
DATA_ROOT = f"s3://tradebot-config-tokyo/data/stage3/{VERSION}-long"
MODEL_BASE_S3 = f"s3://tradebot-config-tokyo/models/xgb-{VERSION}-long"
...
def _merge_hps(base: dict, overlay: dict):
    """Fusionne HP_BASE et une combo de grille, avec casts sûrs pour SageMaker."""
    h = base.copy()
    h.update(overlay)
...
def _json_dumps_safe(payload: dict) -> bytes:
    """Toujours sérialisable (fallback sur types natifs)."""
...
def _require_csv_prefix(fs: s3fs.S3FileSystem, prefix: str, label: str):
    ...
    if not paths:
        raise FileNotFoundError(
            f"[DATA MISSING] Aucun CSV sous {prefix}/part-*.csv ({label})"
        )
    return paths


def read_scale_pos_weight_from_meta():
    """Lit _meta/train_pos_weight.json si présent (vx-long)."""
...
def make_channels():
    """Canaux d’entrainement XGBoost (features + poids + meta)."""
    return {
        "train": TrainingInput(TRN_DIR, content_type="text/csv", input_mode="File"),
        "train_weight": TrainingInput(
            TRW_DIR, content_type="text/csv", input_mode="File"
        ),
        "validation": TrainingInput(
            VAL_DIR, content_type="text/csv", input_mode="File"
        ),
        "validation_weight": TrainingInput(
            VAW_DIR, content_type="text/csv", input_mode="File"
        ),
        "meta": TrainingInput(
            META_DIR, content_type="application/x-directory", input_mode="File"
        ),
    }


def submit_job(hps: dict, session: sagemaker.Session) -> Estimator:
    """Soumet un job XGBoost sur SageMaker (long-only, vx)."""
    ...
    est = Estimator(
        ...
        base_job_name="xgb-long",
        tags=[
            {"Key": "project", "Value": "ml_bot"},
            {"Key": "stage", "Value": "stage3-xgb-longonly"},
            {"Key": "side", "Value": "long"},
        ],
    )
    job_name = f"xgb-long-{time.strftime('%Y%m%d-%H%M%S')}"
...
def _load_csv_dir(fs: s3fs.S3FileSystem, prefix: str) -> pd.DataFrame:
    """Charge tous les part-*.csv d’un préfixe S3 en un seul DataFrame."""
    paths = [f"s3://{p}" for p in fs.glob(prefix.replace("s3://", "") + "/part-*.csv")]
    if not paths:
...
def _maybe_load_weights(fs: s3fs.S3FileSystem, weight_prefix: str):
    """Charge les poids si présents, sinon retourne None."""
    try:
...
def _load_booster_from_tar(fs: s3fs.S3FileSystem, model_uri: str) -> xgb.Booster:
    """Ouvre model.tar.gz (SageMaker) et en extrait le Booster XGBoost."""
    ...
    with tarfile.open(fileobj=io.BytesIO(raw), mode="r:gz") as tar:
        ...
        if member is None:
            raise RuntimeError(
                "model.json / xgboost-model introuvable dans model.tar.gz"
            )
        model_bytes = tar.extractfile(member).read()

    booster = xgb.Booster()
    try:
        # xgboost>=2.0 accepte un buffer
        booster.load_model(io.BytesIO(model_bytes))
    except Exception:
        # fallback via fichier temporaire (compat xgboost<2.0)
...
def evaluate_model(model_uri: str, val_prefix: str, valw_prefix: str):
    """Évalue un modèle sur la validation (AUPRC, F1, threshold F1-max, confusion)."""
    fs = s3fs.S3FileSystem()
    ...
    if np.all(~np.isfinite(f1s)):
        ...
        f1 = float(
            f1_score(y, ybin, sample_weight=sample_weight)
...
def _s3_put_bytes(s3_client, s3_uri: str, data: bytes):
    """Écrit un blob sur S3 à partir d’une URI s3://bucket/key."""
    bkt = s3_uri.split("/", 3)[2]
    key = s3_uri.split("/", 3)[3]
    s3_client.put_object(Bucket=bkt, Key=key, Body=data)


# === Utilitaires d’évaluation TEST & thresholds par poche ====================


def evaluate_split(model_uri: str, prefix_features: str, prefix_weights: str):
    """Charge un split (train/val/test) et renvoie (y, yhat, w)."""
    fs = s3fs.S3FileSystem()
...
def _make_thr_json_from_reports(
    rpt_df: pd.DataFrame, thr_df: pd.DataFrame, auprc_floor: float = 0.10
):
    """
...
def eval_test_and_breakdown(best_model_uri: str):
    """
    Évalue le meilleur modèle sur TEST + breakdown AUPRC par (tf, side_num),
    puis écrit un JSON de thresholds par poche (auprc_floor=0.10).
    """
    fs = s3fs.S3FileSystem()
    ...
    auprc = float(
        average_precision_score(y, yhat, sample_weight=w)
        if w is not None
        else average_precision_score(y, yhat)
    )
    ...
    rpt = (
        mdf.groupby(["tf", "side_num"], as_index=False)
    ...
    for (tf, side_num), g in mdf.groupby(["tf", "side_num"]):
        ...
        thr_rows.append(
            dict(tf=tf, side_num=int(side_num), thr_f1=best_thr, F1=f1)
        )
...
def _per_pocket_reports(model_uri: str):
    ...
    grp = (
        mdf[["tf", "side_num", "y", "yhat"]]
        .groupby(["tf", "side_num"], as_index=False)
        .apply(_summ, include_groups=False)
    )
    rpt_df = grp.reset_index(
        drop=True
    )  # colonnes: tf, side_num, n,pos,pos_rate,AUPRC,thr_f1,F1
...
def make_thresholds_json_from_breakdown(
    ...
    thr_map, ok_keys = _make_thr_json_from_reports(
        rpt_df, thr_df, auprc_floor=auprc_floor
    )
    ...
    payload = {
        ...
        "policy": {
            ...
            "criterion": policy,
            **(
                {"prec_floor": float(prec_floor)}
                if (policy == "prec_floor" and prec_floor is not None)
                else {}
            ),
        },
        "enabled_pockets": sorted(list(ok_keys)),
...
def main():
    ...
    print(
        f"[run] {total} combinaisons à lancer | instance={INSTANCE_TYPE} | spot={bool(SPOT)}"
    )
    ...
    for hp0 in PRIORITY_COMBOS:
        ...
        try:
            ...
            print(
                f"[fit] terminé en {train_secs:.1f}s | model_uri={model_uri}"
            )

            t1 = time.time()
            print("[eval] lecture des CSV validation + poids (si présents)…")
            metrics = evaluate_model(
                model_uri=model_uri, val_prefix=VAL_DIR, valw_prefix=VAW_DIR
            )
    ...
    if ok_rows:
        show = (
            pd.DataFrame(ok_rows)[["AUPRC", "F1", "hps", "model_uri"]]
    ...
    try:
        ...
        if (not features) or (len(features) != expected_feat):
            print(
                f"[manifest] warn: features meta manquantes ou taille != {expected_feat}, "
        ...
        release_name = f"long-{stamp}-{VERSION}"
        ...
        manifest = {
            ...
            "inference": {
                ...
                # objective="binary:logistic" -> sortie = P(Y=1)
            ...
            "data_contract": {
                ...
                "drop_cols": ["side_num"],
                "features": features,
                # stage3 v50 a déjà fait la normalisation -> on n’en refait pas
        ...
        print(
            f"[manifest] WARN: impossible de générer le manifest automatiquement: {e}",
            file=sys.stderr,
        )
