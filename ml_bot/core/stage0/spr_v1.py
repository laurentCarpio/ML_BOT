...
def build_candidates_audit(
    ...
    *,
    ...
    """
    OOM-safe audit of the hard-filter cascade (NO MS / NO cooldown).
    Returns counters + step/cum rates.
    """
    if features is None or len(features) == 0:
        return {"n_start": 0, "n_final": 0, "steps": []}
    ...
    # ensure train-learned thr is available (so range60s mask can use it)
    if "thr_range60s_min_train" not in df.columns:
        df["thr_range60s_min_train"] = float(thr.range60s_min)

    masks = candidate_filter_masks(df, cfg, thr)
        ...
        steps.append(
            {
                ...
                "before": before,
                "after": after,
                "step_rate": float(after / before) if before > 0 else float("nan"),
                "cum_rate": float(after / n0) if n0 > 0 else float("nan"),
            }
        )
        if after == 0:
            break

    return {"n_start": n0, "n_final": int(m.sum()), "steps": steps}


def audit_candidate_filters(
    features: pd.DataFrame,
    cfg: Stage0SPRConfig,
    thr: SPRThresholds,
    *,
    ...
    """
    Convenience wrapper (kept name used by run_stage0_spr_walkforward.py).
    """
    return build_candidates_audit(features, cfg, thr, sample_n=sample_n, seed=seed)
